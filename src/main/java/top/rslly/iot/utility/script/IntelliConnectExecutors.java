...
public class IntelliConnectExecutors {
  /**
   ...
   * may use multiple queues to reduce contention. The parallelism level corresponds to the maximum
   * number of threads actively engaged in, or available to engage in, task processing. The actual
   * number of threads may grow and shrink dynamically. A work-stealing pool makes no guarantees
  ...
  public static ExecutorService newWorkStealingPool(int parallelism, String namePrefix) {
    return new ForkJoinPool(parallelism, new IntelliConnectForkJoinWorkerThreadFactory(namePrefix),
        null, true);
  }
