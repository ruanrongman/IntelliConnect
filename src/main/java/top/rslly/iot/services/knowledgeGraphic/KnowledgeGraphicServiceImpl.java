...
public class KnowledgeGraphicServiceImpl implements KnowledgeGraphicService{
    ...
    public JsonResult<?> getKnowledgeGraphic(KnowledgeGraphicNodeEntity rootNode, int maxDepth) {
        ...
        List<KnowledgeGraphicNodeEntity> nodeList = knowledgeGraphicNodeRepository.findAllByProductUid(rootNode.getProductUid());
        ...
        while(!nodeStack.isEmpty() && maxDepth > 0){
            ...
            for(KnowledgeGraphicNodeEntity node: nodeList){
                List<KnowledgeGraphicRelationEntity> relationList = knowledgeGraphicRelationRepository.getAllByFrom(node.getId());
    ...
    public JsonResult<?> getNodeRelations(String name) {
        ...
        List<KnowledgeGraphicRelationEntity> relations = knowledgeGraphicRelationRepository.getAllByFrom(node.getId());
        return ResultTool.success(relations);
    }

    @Override
    public JsonResult<?> getNodeRelations(long id) {
        List<KnowledgeGraphicRelationEntity> relations = knowledgeGraphicRelationRepository.getAllByTo(id);
        return ResultTool.success(relations);
    }
