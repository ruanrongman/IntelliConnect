...
### 2. Agent 智能体系统
- **大模型集成**: 支持 Qwen3、GLM、DeepSeek v4 等多种模型
- **双模式工具调用**: 支持 ReAct 提示词模式和原生 Function Calling 模式（通过 `ai.agent.mode` 配置）
- **MCP 协议集成**: 支持多端点 WebSocket 连接，可配置端点数量（1-20）和每端点工具上限
- **知识库**: RAG 检索增强生成，支持文档上传和智能检索
- **记忆系统**: 长期记忆管理和对话历史分析
- **历史消息**: 支持对话历史持久化存储和按用户权限查询
- **高质量音频**: 支持可配置音频参数（16/24/48kHz 采样率，20/40/60ms 帧时长），按连接协商
...
- **后端框架**: Spring Boot 3.5.7 + Spring Security
