...
### 11. 历史消息表 (history_message)

存储Agent对话历史消息，支持文本调试界面的历史记录持久化。

| 字段名 | 类型 | 说明 |
|--------|------|------|
| id | INT (PK) | 消息唯一标识，自增主键 |
| chat_id | VARCHAR | 对话ID，关联同一对话的多条消息 |
| request_id | VARCHAR | 请求ID，标识单次交互 |
| sequence_num | INT | 消息序号，同一请求内的消息排序 |
| message_type | VARCHAR | 消息类型（user/assistant/tool等） |
| content | TEXT | 消息内容 |
| time | DATETIME | 消息时间 |
