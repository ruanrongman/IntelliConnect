...
## 高质量音频
本平台支持可配置的高质量音频参数，设备连接时可协商音频参数：
- **采样率**: 支持 16kHz、24kHz、48kHz
- **帧时长**: 支持 20ms、40ms、60ms
- 音频参数根据设备能力按连接动态协商，无需手动配置

## 设备昵称
小智设备支持设置昵称（nickname），便于区分同一产品下的多个设备：
- 昵称在OTA绑定或MCP连接时设置
- 可通过 `GET /api/v2/memoryByNickName` 接口按昵称查询设备记忆